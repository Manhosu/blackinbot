...
export default function BotDashboardPage({ params: paramsPromise }: { params: Promise<{ id: string }> }) {
  ...
  // Removido mediaSource - sempre será upload direto
  ...
  const saveCustomContent = async () => {
    ...
    try {
      ...
      if (mediaType !== 'none') {
        ...
        } else if (customMedia.trim()) {
          // Usar mídia já existente (URL já salva anteriormente)
          finalMediaUrl = customMedia.trim();
          finalMediaType = mediaType;
          console.log('📎 Usando mídia existente:', finalMediaUrl);
        } else {
          // Mídia selecionada mas sem arquivo - apenas remover mídia existente
          console.log('⚠️ Mídia selecionada mas sem arquivo - removendo mídia');
          finalMediaUrl = '';
          finalMediaType = 'none';
        }
      }
